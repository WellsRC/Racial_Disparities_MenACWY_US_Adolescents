...
load('NIS_Teen_Data_Sample.mat','Dropout_Race_Mother_Education','Two_Dose_Race_Mother_Education','U_Race','Yr');
reorder_array=[4 1 3 2];
U_Race=U_Race(reorder_array);
Dropout_Race_Mother_Education=Dropout_Race_Mother_Education(:,reorder_array,:,:);
Two_Dose_Race_Mother_Education=Two_Dose_Race_Mother_Education(:,reorder_array,:,:);

Race={'NH White','Hispanic','NH Other + multiple races','NH African American'}';
Educationv={'No college', 'Some College'}';
...
yyax=[0.62 0.135];
figure('units','normalized','outerposition',[0.2 0.05 0.6 0.6]);

for pir=1:2
    AIC_logit=zeros(length(Model),length(Race));
    Data_Race=squeeze(Two_Dose_Race_Mother_Education(:,:,pir,:));
    Data2_Race=squeeze(Dropout_Race_Mother_Education(:,:,pir,:));
    ...
    Data_Race=squeeze(Dropout_Race_Mother_Education(:,:,pir,:));
    ...
    if(pir==1)
        Table_Dropout=table(Education,Race,Pivot_Year,Coefficient,prob_tred_change);
    else
        Table_Dropout=[Table_Dropout; table(Education,Race,Pivot_Year,Coefficient,prob_tred_change)];
    end
    
    for rr=1:4
        subplot('Position',[0.085+0.2375.*(rr-1) yyax(pir) 0.195 0.32]) 
...
exportgraphics(gcf, ['Figure_Trend_Race_Education.pdf']);
print(gcf, ['Figure_Trend_Race_Education.jpg'],'-djpeg','-r300');
