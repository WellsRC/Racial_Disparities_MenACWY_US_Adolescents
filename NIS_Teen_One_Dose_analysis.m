...
One_Dose_Model_Summary=[array2table(Income_Mat) array2table(Var_Mat) table(avg_delat_AIC,p_AIC)];
One_Dose_Model_Summary.Properties.VariableNames=[Income_Var Var {'Average_delat_AIC'} {'Probability_Best_Model'}];

[~,s_indx]=sort(p_AIC);

One_Dose_Model_Summary=One_Dose_Model_Summary(s_indx,:);

save('NIS_Teen_Model_Selection_One_Dose_All_Races.mat','One_Dose_Model_Summary');
